...
use crate::metrics::{
    ...
    PROCESSED_LATENCY_IN_MILLISECS, PROCESSED_LATENCY_IN_MILLISECS_ALL, PROCESSED_VERSIONS_COUNT,
};
use aptos_indexer_grpc_utils::{
    ...
    time_diff_since_pb_timestamp_in_milliseconds, EncodedTransactionWithVersion,
};
    ...
    async fn raw_datastream(
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let data_latency_in_ms = first_transaction_in_batch
                    .timestamp
                    .map(time_diff_since_pb_timestamp_in_milliseconds);
                ...
                match tx.try_send(Result::<RawDatastreamResponse, Status>::Ok(resp_item)) {
                    Ok(_) => {
                        ...
                        if let Some(data_latency_in_ms) = data_latency_in_ms {
                            PROCESSED_LATENCY_IN_MILLISECS
                                .with_label_values(&[
                                    request_metadata.request_token.as_str(),
                                    request_metadata.request_name.as_str(),
                                ])
                                .set(data_latency_in_ms as i64);
                            PROCESSED_LATENCY_IN_MILLISECS_ALL
                                .with_label_values(&[request_metadata.request_source.as_str()])
                                .observe(data_latency_in_ms as f64);
                        }
                    },
