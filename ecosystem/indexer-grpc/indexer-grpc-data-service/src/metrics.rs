...
pub static ERROR_COUNT: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "indexer_grpc_data_service_error_count",
        "Number of errors that data service has encountered",
        &["error_type"]
    )
    .unwrap()
...
pub static PROCESSED_LATENCY_IN_MILLISECS: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        "indexer_grpc_data_service_data_latency",
...
pub static PROCESSED_LATENCY_IN_MILLISECS_ALL: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "indexer_grpc_data_service_data_latency_all",
        "Latency of data service based on latest processed transaction",
        &["request_token"]
    )
    .unwrap()
