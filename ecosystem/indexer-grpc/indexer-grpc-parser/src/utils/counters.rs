...
pub static PROCESSOR_DATA_RECEIVED_LATENCY: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        "indexer_processor_data_receive_latency",
        "Data latency when processor receives transactions",
...
pub static PROCESSOR_DATA_PROCESSED_LATENCY_IN_MILLISECS: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        "indexer_processor_data_processed_latency",
        "Data latency when processor finishes processing transactions",
        &["request_token", "processor_name"]
    )
    .unwrap()
